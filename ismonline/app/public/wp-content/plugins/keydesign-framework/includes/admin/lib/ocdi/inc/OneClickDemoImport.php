  ...
  private $plugin_page_setup = array();
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
			...
			'page_title' => esc_html__( 'Starter Sites', 'pt-ocdi' ),
			'menu_title' => esc_html__( 'Starter Sites', 'pt-ocdi' ),
			'capability' => 'import',
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						...
						'dialog_title'          => esc_html__( 'Import', 'pt-ocdi' ),
						'dialog_no'             => esc_html__( 'Cancel', 'pt-ocdi' ),
						'dialog_yes'            => esc_html__( 'Import', 'pt-ocdi' ),
	...
	public function import_demo_data_ajax_callback() {
		...
		$importForm = false;
		if ( !empty( $_POST['importForm'] ) ) {
			$importForm = json_decode( wp_specialchars_decode( stripslashes($_POST['importForm']) ) );
		}
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->selected_import_files = Helpers::download_import_files( $this->import_files[ $this->selected_index ], $importForm );
		...
		if ( ! empty( $this->selected_import_files['innerPages'] ) ) {
			$this->append_to_frontend_error_messages( $this->importer->import_content( $this->selected_import_files['innerPages'] ) );
		}
