jQuery( function ( $ ) {
	...
	(function () {
		...
		$( '.ocdi__gl-navigation li' ).first().addClass( 'active' );
	...
	function gridLayoutImport( selectedImportID, $itemContainer ) {
		...
		$itemContainer.closest( '.js-ocdi-gl' ).prepend( '<h3>' + ocdi.texts.selected_import_title + '</h3>' );
		...
		if( $('.keydesign-import-form').length > 0 ) {
			if( $('#keydesign_import_homepage').is(":checked") ){
				importForm.homepage = true;
			}
			if( $('#keydesign_import_innerpages').is(":checked") ){
				importForm.innerPages = true;
			}
			// if( $('#keydesign_import_themeoptions').is(":checked") ){
				importForm.themeOptions = true;
			// }
			// if( $('#keydesign_import_widgets').is(":checked") ){
				importForm.widgets = true;
			// }
		}
		...
		data.append( 'importForm', JSON.stringify( importForm ) );
	...
	function displayImportForm( data ) {
		...
		if ( data.local_import_file ) {
			outputForm += '<div class="keydesign-import-form hide-all">';
			outputForm += '<p class="import-form-subtitle">Choose what content to import. Once you\'ve made your selections, click "Import" to start the process.</p>';

			if ( data.local_import_file ) {
				outputForm += '<label for="keydesign_import_homepage"><input name="keydesign_import_homepage" type="checkbox" id="keydesign_import_homepage" value="homepage" checked="checked">Homepage</label>';
			}

			if ( data.local_import_innerpages_file ) {
				outputForm += '<label for="keydesign_import_innerpages"><input name="keydesign_import_innerpages" type="checkbox" id="keydesign_import_innerpages" value="innerPages" checked="checked">Inner pages</label>';
			}
			//
			// if ( data.local_import_redux ) {
			// 	outputForm += '<label for="keydesign_import_themeoptions"><input name="keydesign_import_themeoptions" type="checkbox" id="keydesign_import_themeoptions" value="themeOptions" checked="checked">Theme options</label>';
			// }
			//
			// if ( data.local_import_widget_file ) {
			// 	outputForm += '<label for="keydesign_import_widgets"><input name="keydesign_import_widgets" type="checkbox" id="keydesign_import_widgets" value="widgets" checked="checked">Widgets</label>';
			// }
	...
	function displayConfirmationPopup( selectedImportID, $itemContainer ) {
		var importForm = displayImportForm(ocdi.import_files[ selectedImportID ]);
