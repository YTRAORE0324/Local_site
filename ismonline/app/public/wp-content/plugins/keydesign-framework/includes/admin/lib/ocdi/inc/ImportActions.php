...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action( 'pt-ocdi/after_content_import_execution', array( $this, 'nuxy_import' ), 30, 3 );
	...
	/**
	 * Execute the Nuxy import.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, nuxy).
	 * @param array $import_files          The filtered import files defined in `pt-ocdi/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	 */
	public function nuxy_import( $selected_import_files ) {
		...
		if ( ! empty( $selected_import_files['redux'] ) ) {
			$theme_options = $selected_import_files['redux'][0]['file_path'];
			$option_name = $selected_import_files['redux'][0]['option_name'];
			$encode_options = $wp_filesystem->get_contents( $theme_options );
			$import_options = json_decode( $encode_options, true );
			update_option( $option_name, $import_options );
		}
	}
